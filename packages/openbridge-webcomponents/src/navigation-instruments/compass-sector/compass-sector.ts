...
export class ObcCompassSector extends LitElement {
  ...
  override willUpdate(changed: PropertyValues): void {
    ...
    if (this.zoomToFitArc) {
      // Zoomed: 1:1 scale while content fits inside 120°, then cap at
      // 120° and switch to FOV compression for wider angles.
      ...
      // Not zoomed: fixed 120° arc, FOV scaling compresses compass degrees
  ...
  private _mapAngle(compassDeg: number): number {
    return this.heading + (compassDeg - this.heading) * this._scale;
  }

  // ---------------------------------------------------------------------------
  // Areas — fixed 120° arc centered on heading
  // ---------------------------------------------------------------------------

  private get _areas(): WatchArea[] {
    return [
      {
  ...
  private get _tickmarks(): Tickmark[] {
  ...
  private get _angleAdviceRaw(): AngleAdviceRaw[] {
    ...
    return html`
      <div class="container">
        <obc-watch
          .touching=${this.touching}
          .padding=${PADDING}
          .advices=${this._angleAdviceRaw}
          .tickmarks=${this._tickmarks}
